...
export const FormImageStep = () => {
	...
	const currentSneakerSize = useMemo(() => {
		return currentSneaker
			? getSizeForCurrentUnit(currentSneaker).toString()
			: '';
	}, [currentSneaker, getSizeForCurrentUnit]);

	const handleSneakerToAddUpdate = useCallback(
		(newData: SneakerFormData) => {
	...
	const { control, reset, watch, displayedError, getFieldErrorWrapper } =
		useFormController<SneakerFormData>({
			...
			defaultValues: {
				...
				condition:
					...
					currentSneaker?.condition?.toString() ||
					'',
				price_paid:
					sneakerToAdd?.price_paid ||
					currentSneaker?.price_paid?.toString() ||
					'',
				description:
					sneakerToAdd?.description ||
			...
			if (value.images) {
				...
				handleSneakerToAddUpdate({
					...
					images: (value.images || []).filter(
						(img): img is SneakerPhoto =>
							img != null && img.uri != null
					),
				});
			}
		});
		...
		if (
			...
			const formData = {
				...
				condition:
					...
					'',
				price_paid:
					currentSneakerToAdd.price_paid ||
					currentSneaker?.price_paid?.toString() ||
					'',
				description:
					currentSneakerToAdd.description ||
					currentSneaker?.description ||
					'',
				og_box:
					currentSneakerToAdd.og_box ||
					currentSneaker?.og_box ||
					false,
				ds: currentSneakerToAdd.ds || currentSneaker?.ds || false,
				is_women:
					currentSneakerToAdd.is_women ||
					currentSneaker?.gender === 'women' ||
					false,
		...
		<View className="flex-1 p-4">
			<View className="mb-6">
				...
				<Text className="font-open-sans text-center text-gray-600">
					{currentSneaker
						? t('collection.modal.descriptions.editImages')
