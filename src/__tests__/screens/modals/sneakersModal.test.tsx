...
describe('SneakersModal', () => {
	...
	describe('Modal visibility', () => {
		it('should not render if isVisible is false', () => {
			mockUseModalStoreFn.mockReturnValue({
				isVisible: false,
			});
	...
	describe('InitialStep (index)', () => {
		...
		it('should render InitialStep with correct content', () => {
			...
			expect(
				screen.getByText(/how do you want to proceed/i)
			).toBeOnTheScreen();
		});
	...
	describe('FormDetailsStep (addFormDetails)', () => {
		...
		it('should render form fields with empty values initially', () => {
			...
			expect(screen.getByTestId('brand-input-value').props.children).toBe(
				'Nike'
			);
			expect(
				screen.getByTestId('status-input-value').props.children
			).toBe('Rocking');
		...
		describe('form focus', () => {
			it('should display fields with orange border on focus', async () => {
				...
				expect(modelInput.props.className).toContain(
					'border-2 border-orange-500'
				);
				expect(brandInput.props.className).toContain(
					'border-2 border-primary'
				);
				expect(statusInput.props.className).toContain(
					'border-2 border-primary'
				);
				expect(sizeInput.props.className).toContain(
					'border-2 border-orange-500'
				);
				expect(conditionInput.props.className).toContain(
					'border-2 border-orange-500'
				);
				expect(priceInput.props.className).toContain(
					'border-2 border-orange-500'
				);
				expect(descriptionInput.props.className).toContain(
					'border-2 border-orange-500'
				);
			});
		});
		...
		describe('form validation', () => {
			describe('model field validation', () => {
				it('should display error if model is less than 2 characters on blur', async () => {
					...
					expect(errorMessage.props.children).toBe(
						'Sneaker model must be at least 2 characters long.'
					);
				});
				...
				it('should display error if model contains brand name on blur', async () => {
					...
					const errorMessage = screen.getByTestId('error-message');
					expect(errorMessage.props.children).toBe(
						'The model cannot contain the brand.'
					);
				});
			});
			...
			describe('size field validation', () => {
				it('should display error if size is less than 3 on blur', async () => {
					...
					expect(errorMessage.props.children).toBe(
						'Size must be between 7 and 15 (US).'
					);
				});
