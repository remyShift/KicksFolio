...
import { useBugReportStore, BugReportFormData } from '@/store/useBugReportStore';
...
export const BugReportModal: React.FC = () => {
	...
	const { handleReportBugSubmit, priorityOptions, colorConfig } = useBugReport();
	const {
		isVisible,
		isLoading,
		errorMsg,
    ...
    const formController = useFormController<BugReportFormData>({
        ...
        fieldNames: ['title', 'description', 'stepsToReproduce', 'expectedBehavior', 'actualBehavior', 'priority'],
						...
						<View className="mb-4">
							...
							<View className="flex-row gap-4">
								{priorityOptions.map((option) => {
									const isSelected = watch('priority') === option.value;
									...
									return (
										<TouchableOpacity
											key={option.value}
											onPress={() => setValue('priority', option.value as 'low' | 'medium' | 'high')}
